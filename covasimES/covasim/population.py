...
def make_people(sim, popdict=None, die=True, reset=False, recreate=False, verbose=None, **kwargs):
    ...
    if isinstance(popdict, cvppl.People) and not recreate:
        ...
        people = cvppl.People(sim.pars, uid=popdict['uid'], age=popdict['age'], sex=popdict['sex'],race=popdict['race'], contacts=popdict['contacts']) # List for storing the people
...
def validate_popdict(popdict, pars, verbose=True):
    ...
    required_keys = ['uid', 'age', 'sex','race']
...
def make_randpop(pars, use_age_data=True, use_household_data=True, sex_ratio=0.5, microstructure='random', **kwargs):
    ...
    # Handle race for random population
    races          = np.random.choice([0,1,2,3,4], p=[0.60208333, 0.12604167, 0.009375  , 0.06770833, 0.19479167], size=pop_size)
...
def make_random_contacts(pop_size, n, overshoot=1.2, dispersion=None, mapping=None):
    ...
    if dispersion is None:
        p_count = cvu.n_poisson(n, pop_size) # Draw the number of Poisson contacts for each person
    else:
