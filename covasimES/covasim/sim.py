...
class Sim(cvb.BaseSim):
    ...
    **Examples**::
    ...
    def finalize(self, verbose=None, restore_pars=True):
        ...
        progs=cvpar.get_prognoses()
        age_cutoffs = progs['age_cutoffs']
        m_totals=np.zeros((6,5))
        h_totals =np.zeros((6,5))
        p_totals = np.zeros((6,5))
        s_totals = np.zeros((6,5))
        i_totals = np.zeros((6,5))
        e_totals = np.zeros((6, 5))
        icu_totals = np.zeros((6,5))


        for k in ["dead","severe","symptomatic","exposed","infectious","critical","all"]:
            if k == "all":
                a_arr = self.people['age']
                r_arr = self.people['race']
            else:
                a_arr = self.people['age'][~np.isnan(self.people['date_'+k])]
                r_arr = self.people['race'][~np.isnan(self.people['date_'+k])]
            for ab in range(6):
                lb = age_cutoffs[ab]
                if ab<=4:
                    ub = age_cutoffs[ab+1]
                    ...
                    ub = 101
                r_of_a = r_arr[np.where((a_arr>=lb) & (a_arr <ub))]
                if k =="dead":
                    for r in range(5):
                        m_totals[ab,r] += len(r_of_a[np.where(r_of_a==r)])
                elif k =="severe":
                    for r in range(5):
                        h_totals[ab, r] += len(r_of_a[np.where(r_of_a == r)])
                elif k =="symptomatic":
                    for r in range(5):
                        s_totals[ab,r] += len(r_of_a[np.where(r_of_a==r)])
                elif k =="exposed":
                    for r in range(5):
                        e_totals[ab, r] += len(r_of_a[np.where(r_of_a == r)])
                elif k =="infectious":
                    for r in range(5):
                        i_totals[ab, r] += len(r_of_a[np.where(r_of_a == r)])
                elif k =="critical":
                    for r in range(5):
                        icu_totals[ab, r] += len(r_of_a[np.where(r_of_a == r)])
                else:
                    for r in range(5):
                        p_totals[ab, r] += len(r_of_a[np.where(r_of_a == r)])



        self.results['mortality_by_race_and_age'] = m_totals
        self.results['hospitalization_by_race_and_age'] = h_totals
        self.results['icu_by_race_and_age'] = icu_totals
        self.results['infections_by_race_and_age'] = i_totals
        self.results['symptomatic_infections_by_race_and_age'] = s_totals
        self.results['race_and_age'] = p_totals
