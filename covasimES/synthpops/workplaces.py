...
def get_workplace(pop, wpid):
    ...
    if len(pop.workplaces) <= wpid:
        raise IndexError(
            f"Workplace id (wpid): {wpid} out of range. There are {len(pop.workplaces)} workplaces stored in this object.")
...
def get_uids_potential_workers(student_uid_lists, employment_rates, age_by_uid, race_by_uid):
    """
    ...
    Args:
        ...
        race_by_uid (dict)        : A dictionary mapping ID to race for individuals in the population.
    ...
    potential_worker_uids_by_age_and_race = {}
    potential_worker_ages_left_count = {}

    for r in range(5):
        potential_worker_uids_by_age_and_race[r] = {}
        for a in range(101):
            if a >= 15:
                potential_worker_uids_by_age_and_race[r][a] = []
    ...
    for uid in potential_worker_uids:
        ...
        if ai in potential_worker_uids_by_age.keys():
            ...
            potential_worker_uids_by_age_and_race[ri][ai].append(uid)
    ...
    return potential_worker_uids, potential_worker_uids_by_age, potential_worker_ages_left_count, potential_worker_uids_by_age_and_race
...
def assign_rest_of_workers(workplace_sizes, potential_worker_uids, potential_worker_uids_by_age,
                           potential_worker_uids_by_age_and_race, workers_by_age_to_assign_count, age_by_uid,
                           race_by_uid, age_brackets, age_by_brackets, contact_matrices, workplace_rd_by_frontline):
    """
    ...
    workplace_age_lists = []
    workplace_uid_lists = []
    rprobs_lfr = np.array(workplace_rd_by_frontline[0])
    rprobs_hfr = np.array(workplace_rd_by_frontline[1])
    rprobs_nfr = np.array(workplace_rd_by_frontline[2])
    ...
    for n, size in enumerate(workplace_sizes):
        ...
        if (size > 50 and size < 100):
            rchoice = np.random.choice(np.arange(5), p=rprobs_hfr)
        elif size >= 100:
            rchoice = np.random.choice(np.arange(5), p=rprobs_lfr)
        else:
            rchoice = np.random.choice(np.arange(5), p=rprobs_nfr)

        while len(potential_worker_uids_by_age_and_race[rchoice][aindex]) < 1:
            if (size > 50 and size < 100):
                rchoice = np.random.choice(np.arange(5), p=rprobs_hfr)
            elif size >= 100:
                rchoice = np.random.choice(np.arange(5), p=rprobs_lfr)
        ...
        uid = potential_worker_uids_by_age_and_race[rchoice][aindex][0]

        potential_worker_uids_by_age[aindex].remove(uid)
        potential_worker_uids_by_age_and_race[rchoice][aindex].remove(uid)
        potential_worker_uids.pop(uid, None)
        ...
        if len(potential_worker_uids) <= size or workers_left_count <= size:
            for ai in workers_by_age_to_assign_count:
                for i in range(workers_by_age_to_assign_count[
                                   ai]):  # do not change this during the loop but afterwards, and if 0 then no one will be placed
                    uid = potential_worker_uids_by_age[ai][0]
                    ri = race_by_uid[uid]
                    new_work.append(ai)
                    new_work_uids.append(uid)
                    potential_worker_uids_by_age[ai].remove(uid)
                    potential_worker_uids_by_age_and_race[ri][ai].remove(uid)
                    potential_worker_uids.pop(uid, None)
                workers_by_age_to_assign_count[
                    ai] = 0  # set to zero now that everyone will be placed in this last workplace
            workers_by_age_to_assign_distr = spb.norm_dic(workers_by_age_to_assign_count)
        else:
            for i in range(1, size):
                ...
                workers_left_in_bracket = [workers_by_age_to_assign_count[a] for a in age_brackets[bi] if
                                           len(potential_worker_uids_by_age[a]) > 0]
                ...
                if np.sum(b_prob):  # pragma: no cover
                    ...
                    for r in range(5):
                        if len(potential_worker_uids_by_age_and_race[r][ai]) < 1:
                            rprobs_hfr[r] = 0
                            rprobs_hfr = rprobs_hfr / sum(rprobs_hfr)
                            rprobs_lfr[r] = 0
                            rprobs_lfr = rprobs_lfr / sum(rprobs_lfr)
                            rprobs_nfr[r] = 0
                            rprobs_nfr = rprobs_nfr / sum(rprobs_nfr)

                    if (size > 50 and size < 100):
                        ri = np.random.choice(np.arange(5), p=rprobs_hfr)
                    elif size >= 100:
                        ri = np.random.choice(np.arange(5), p=rprobs_lfr)
                    else:
                        ri = np.random.choice(np.arange(5), p=rprobs_nfr)

                    uid = potential_worker_uids_by_age_and_race[ri][ai][0]
                    new_work.append(ai)
                    new_work_uids.append(uid)
                    potential_worker_uids_by_age[ai].remove(uid)
                    potential_worker_uids_by_age_and_race[ri][ai].remove(uid)
    ...
    return workplace_age_lists, workplace_uid_lists, potential_worker_uids, potential_worker_uids_by_age, potential_worker_uids_by_age_and_race, workers_by_age_to_assign_count
