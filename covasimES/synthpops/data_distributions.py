...
def read_race_prob_hh(datadir=None, location=None, state_location=None, country_location=None, nbrackets=None, file_path=None, use_default=False):
    """
    A dict of the race probability by household size. If use_default, then we'll
    first try to look for location specific data and if that's not available
    we'll use default data from settings.location,
    settings.state_location, settings.country_location. This may not
    be appropriate for the population under study so it's best to provide as
    ...
    Returns:
        dict: A dictionary of the race probability by household size. Keys map to a
        probability of each of the 5 racial groups for that household size.

    """
    # Use default if no file for this location.
    location_data = load_location(location, state_location, country_location, revert_to_default=use_default)
    race_probs = location_data.race_probability_by_hh_size
    # Use default if no data for this parameter.
    if use_default and (race_probs is None or len(race_probs) == 0):
        return read_race_prob_hh(location=defaults.settings.location,
                                      state_location=defaults.settings.state_location,
                                      country_location=defaults.settings.country_location,
                                      use_default=False)
    percent = [race_prob for race_prob in race_probs]
    r = dict(zip(np.arange(1,len(race_probs)+1), percent))
    return r
...
def get_school_racial_distributions(datadir=None, location=None, state_location=None, country_location=None, file_path=None, use_default=False):
    """
    Get dictionary of examples of school racial_distributions. If use_default, then we'll first
    try to look for location specific data and if that's not available we'll use
    default data from settings.location, settings.state_location,
    settings.country_location. This may not be appropriate for the
    population under study so it's best to provide as much data as you can for
    ...
    location_data = load_location(location, state_location, country_location, revert_to_default=use_default)
    # Use default if no data for this parameter.
    if use_default and (location_data.school_racial_distributions_by_type is None or len(location_data.school_racial_distributions_by_type) == 0):
        return get_school_enrollment_rates(location=defaults.settings.location,
                                           state_location=defaults.settings.state_location,
                                           country_location=defaults.settings.country_location,
                                           use_default=False)

    dist = [ [d['school_type'], d['dist']] for d in location_data.school_racial_distributions_by_type]
    return dict(dist)
...
def get_workplace_race_distr_by_frontline_status(datadir=None, location=None, state_location=None, country_location=None, file_path=None, use_default=False):
    """
        Get workplace racial distributions for high and low risk groups. If use_default, then we'll first try to look
        for location specific data and if that's not available we'll use default
        data from settings.location, settings.state_location,
        settings.country_location. This may not be appropriate for the
        population under study so it's best to provide as much data as you can for
        ...
        Returns:
            dict: A dictionary of workplace racial distributions by frontline status.
        """
    # Use default if no file for this location.
    location_data = load_location(location, state_location, country_location, revert_to_default=use_default)
    # Use default if no data for this parameter.
    if use_default and (location_data.workplace_size_counts_by_num_personnel is None or len(
            location_data.workplace_size_counts_by_num_personnel) == 0):
        return get_workplace_size_brackets(location=defaults.settings.location,
                                           state_location=defaults.settings.state_location,
                                           country_location=defaults.settings.country_location,
                                           use_default=False)

    workplace_race_dist = dict()
    for bracket_index, bracket in enumerate(location_data.race_distribution_by_frontline_status):
        workplace_race_dist[bracket_index] = np.array(bracket )
    return workplace_race_dist
