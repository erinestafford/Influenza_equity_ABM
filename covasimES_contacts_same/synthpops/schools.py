...
def get_uids_in_school(datadir, n, location, state_location, country_location, age_by_uid=None,race_by_uid=None, homes_by_uids=None, folder_name=None, use_default=False):
    """
    ...
    uids_in_school_by_race = {0:[],1:[],2:[],3:[],4:[]}
    uids_in_school_by_age_and_race = {0:{},1:{},2:{},3:{},4:{}}
    ...
    for a in np.arange(101):
        ...
        for r in np.arange(5):
            uids_in_school_by_age_and_race[r][a]=[]
        ...
        for uid in home:
            ...
            if rates[a] > 0:
                ...
                if b:
                    ...
                    uids_in_school_by_race[r].append(uid)
                    uids_in_school_by_age_and_race[r][a].append(uid)
                    ages_in_school_count[a] += 1

    return uids_in_school, uids_in_school_by_age,uids_in_school_by_race,uids_in_school_by_age_and_race, ages_in_school_count


def send_students_to_school_with_school_types(school_size_distr_by_type, school_size_brackets, uids_in_school, uids_in_school_by_age, uids_in_school_by_race, uids_in_school_by_age_and_race,ages_in_school_count, school_types_distr_by_age, school_race_dists, school_type_age_ranges):
    """
    ...
    student_uid_lists = []
    school_types = []
    school_race_dists_options=school_race_dists.copy()

    sorted_size_brackets = sorted(school_size_brackets.keys())
    races_in_school_count = {0:sum(uids_in_school_by_race[0]),1:sum(uids_in_school_by_race[1]),2:sum(uids_in_school_by_race[2]),3:sum(uids_in_school_by_race[3]),4:sum(uids_in_school_by_race[4])}
    races_in_school_distr=spb.norm_dic(races_in_school_count)
    ages_in_school_distr = spb.norm_dic(ages_in_school_count)
    age_keys = list(ages_in_school_count.keys())
    race_keys = list(races_in_school_count.keys())
    while len(uids_in_school):
        new_student_ages = []
        new_student_uids = []
        new_student_races = []
        #choosing reference student
        aindex = age_keys[spsamp.fast_choice(ages_in_school_distr.values())]
        while len(uids_in_school_by_age[aindex])<1:
            aindex = age_keys[spsamp.fast_choice(ages_in_school_distr.values())]
        rindex = race_keys[spsamp.fast_choice(races_in_school_distr.values())]
        while len(uids_in_school_by_age_and_race[rindex][aindex]) < 1:
            rindex = race_keys[spsamp.fast_choice(races_in_school_distr.values())]

        uid = uids_in_school_by_age_and_race[rindex][aindex][0]
        uids_in_school_by_age_and_race[rindex][aindex].remove(uid)
        uids_in_school_by_age[aindex].remove(uid)
        uids_in_school_by_race[rindex].remove(uid)
        uids_in_school.pop(uid, None)
        ages_in_school_count[aindex] -= 1
        races_in_school_count[rindex] -= 1
        ages_in_school_distr = spb.norm_dic(ages_in_school_count)
        races_in_school_distr = spb.norm_dic(races_in_school_count)

        new_student_ages.append(aindex)
        new_student_uids.append(uid)
        new_student_races.append(rindex)
        ...
        #racial dist
        school_distr_pos = np.array(school_race_dists_options[school_type])
        school_num = np.argmax(np.array(school_distr_pos)[:,rindex])
        school_distr=school_distr_pos[school_num]
        ...
        #choose rest of students
        potential_student_ages = []
        for a in school_type_age_range:
            potential_student_ages.extend([a] * ages_in_school_count[a])

        if size >= len(potential_student_ages): #if size is bigger than num students possible, make size equivalent to len
            ...
            if (size < school_size_brackets[0][0]) & (len(other_schools) > 0):
                ...
                for n, a in enumerate(school_type_age_range):
                    ...
                    #ADDED
                    for i in range(5):
                        count_r = len(uids_in_school_by_age_and_race[i][a])
                        races_in_school_count[i] -= count_r
                ...
                for n, a in enumerate(school_type_age_range): #adds all
                    count = len(uids_in_school_by_age[a])
                    school_uids_in_age = uids_in_school_by_age[a]
                    new_student_ages.extend([a for i in range(count)])
                    new_student_uids.extend(school_uids_in_age)
                    ...
                    # ADDED
                    for i in range(5):
                        count = len(uids_in_school_by_age_and_race[i][a])
                        races_in_school_count[i] -= count
        ...
        else: # choosing students - if school large enough
            ...
            for n, a in enumerate(school_type_age_range):
                ...
                #get race count based on school_distr
                #if len(uids_in_school_by_age[a])>0:
                #    prop_race_in_age_left =np.array([
                #            len(uids_in_school_by_age_and_race[0][a]) / len(uids_in_school_by_age[a]),
                #            len(uids_in_school_by_age_and_race[1][a]) / len(uids_in_school_by_age[a]),
                #            len(uids_in_school_by_age_and_race[2][a]) / len(uids_in_school_by_age[a]),
                #            len(uids_in_school_by_age_and_race[3][a]) / len(uids_in_school_by_age[a]),
                #            len(uids_in_school_by_age_and_race[4][a]) / len(uids_in_school_by_age[a])])
                #    r_probs = (school_distr*prop_race_in_age_left)/sum(school_distr*prop_race_in_age_left)
                #else:
                r_probs=school_distr
                chosen_r = np.random.choice(race_keys, size=count, replace=True, p=r_probs)
                age_race_count = Counter(chosen_r)
                for r in race_keys:
                    if len(uids_in_school_by_age_and_race[r][a])>=age_race_count[r]:
                        school_uids_in_age_and_race = uids_in_school_by_age_and_race[r][a][:age_race_count[r]]
                        uids_in_school_by_age_and_race[r][a] = uids_in_school_by_age_and_race[r][a][age_race_count[r]:]
                        uids_in_school_by_age[a]=[i for i in uids_in_school_by_age[a] if i not in school_uids_in_age_and_race]
                        uids_in_school_by_race[r]=[i for i in uids_in_school_by_race[r] if i not in school_uids_in_age_and_race]
                        new_student_uids += school_uids_in_age_and_race
                        races_in_school_count[r] -= age_race_count[r]
                    else:
                        new_c = len(uids_in_school_by_age_and_race[r][a])
                        school_uids_in_age_and_race = uids_in_school_by_age_and_race[r][a][:new_c]
                        uids_in_school_by_age_and_race[r][a] = uids_in_school_by_age_and_race[r][a][new_c:]
                        uids_in_school_by_age[a] = [i for i in uids_in_school_by_age[a] if i not in school_uids_in_age_and_race]
                        uids_in_school_by_race[r] = [i for i in uids_in_school_by_race[r] if i not in school_uids_in_age_and_race]
                        new_student_uids += school_uids_in_age_and_race
                        races_in_school_count[r] -= new_c
                        #fill in rest of count
                        for i in range(age_race_count[r]-new_c):
                            new_r = spsamp.fast_choice(r_probs)
                            while len(uids_in_school_by_age_and_race[new_r][a])<1:
                                new_r = spsamp.fast_choice(r_probs)
                            school_uids_in_age_and_race = uids_in_school_by_age_and_race[new_r][a][:1]
                            uids_in_school_by_age_and_race[new_r][a] = uids_in_school_by_age_and_race[new_r][a][1:]
                            uids_in_school_by_age[a] = [i for i in uids_in_school_by_age[a] if i not in school_uids_in_age_and_race]
                            uids_in_school_by_race[r] = [i for i in uids_in_school_by_race[r] if i not in school_uids_in_age_and_race]
                            new_student_uids += school_uids_in_age_and_race
                            races_in_school_count[r] -= 1
...
def assign_teachers_to_schools(student_age_lists, student_uid_lists, employment_rates, workers_by_age_to_assign_count, potential_worker_uids, potential_worker_uids_by_age, potential_worker_uids_by_age_and_race, potential_worker_ages_left_count,race_by_uid, average_student_teacher_ratio=20, teacher_age_min=25, teacher_age_max=75):
    """
    ...
    for n in range(len(student_age_lists)):
        ...
        for nt in range(nteachers):
            ...
            potential_worker_uids_by_age_and_race[ri][a].remove(uid)
    ...
    return teacher_age_lists, teacher_uid_lists, potential_worker_uids, potential_worker_uids_by_age,potential_worker_uids_by_age_and_race, workers_by_age_to_assign_count


def assign_additional_staff_to_schools(student_uid_lists, teacher_uid_lists, workers_by_age_to_assign_count, potential_worker_uids, potential_worker_uids_by_age,potential_worker_uids_by_age_and_race, potential_worker_ages_left_count,race_by_uid, average_student_teacher_ratio=20, average_student_all_staff_ratio=15, staff_age_min=20, staff_age_max=75, with_non_teaching_staff=False):
    """
    ...
    if not with_non_teaching_staff:
        ...
        return non_teaching_staff_uid_lists, potential_worker_uids, potential_worker_uids_by_age,potential_worker_uids_by_age_and_race, workers_by_age_to_assign_count
