...
class Pop(sc.prettyobj):
    ...
    def __init__(self,
                 ...
                 location=None,
                 sheet_name=None,
                 household_method='infer_ages', #trying
                 smooth_ages=True, #trying
                 window_length=7,
                 do_make=True
                 ):
        '''
    ...
    def generate(self):
        ...
        #load race probs by hh
        race_probs_by_hh = spdata.read_race_prob_hh(**loc_pars)
        self.race_probs_by_hh = race_probs_by_hh

        #examples of racial distributions in schools
        school_race_dists = spdata.get_school_racial_distributions(**loc_pars)
        self.school_race_dists  = school_race_dists
        ...
        homes_by_uids, age_by_uid, race_by_uid = sphh.assign_uids_by_homes(homes,race_probs_by_hh)  # include facilities to assign ids
        age_by_uid_arr = np.array([age_by_uid[i] for i in range(self.n)], dtype=int)
        race_by_uid_arr = np.array([race_by_uid[i] for i in range(self.n)], dtype=int)
        self.age_by_uid = age_by_uid_arr
        self.race_by_uid = race_by_uid_arr

        facilities_by_uid_lists = homes_by_uids[0:len(facilities)]

        # Generate school sizes - here
        ...
        #SHOULD STUDENT ENROLLMENT RATES DEPEND ON RACE?
        uids_in_school, uids_in_school_by_age,uids_in_school_by_race,uids_in_school_by_age_and_race,ages_in_school_count = spsch.get_uids_in_school(datadir, n_nonltcf, location, state_location, country_location, age_by_uid, race_by_uid, homes_by_uids, use_default=use_default)  # this will call in school enrollment rates
        ...
        if with_school_types:
            ...
            student_age_lists, student_uid_lists, school_types = spsch.send_students_to_school_with_school_types(school_size_distr_by_type,
                                                                                                                 ...
                                                                                                                 uids_in_school_by_race,
                                                                                                                 uids_in_school_by_age_and_race,
        ...
        res = []
        for slist in student_uid_lists:
            i = 0
            length = len(slist)
            c = 0
            for s in slist:
                c = c+race_by_uid[s]
            res.append(abs(length - c))
            i = i+1
        ...
        potential_worker_uids, potential_worker_uids_by_age, potential_worker_ages_left_count,potential_worker_uids_by_age_and_race = spw.get_uids_potential_workers(student_uid_lists,
                                                                                                                               employment_rates,
                                                                                                                               age_by_uid, race_by_uid)
        ...
        potential_worker_uids, potential_worker_uids_by_age,potential_worker_uids_by_age_and_race, workers_by_age_to_assign_count = spltcf.remove_ltcf_residents_from_potential_workers(facilities_by_uid_lists,
        ...
        teacher_age_lists, teacher_uid_lists, potential_worker_uids, potential_worker_uids_by_age,potential_worker_uids_by_age_and_race, workers_by_age_to_assign_count  = spsch.assign_teachers_to_schools(student_age_lists,
                                                                                                                                                                     student_uid_lists,
                                                                                                                                                                     employment_rates,
                                                                                                                                                                     workers_by_age_to_assign_count,
                                                                                                                                                                     potential_worker_uids,
        ...
        non_teaching_staff_uid_lists, potential_worker_uids, potential_worker_uids_by_age,potential_worker_uids_by_age_and_race, workers_by_age_to_assign_count = spsch.assign_additional_staff_to_schools(student_uid_lists,
                                                                                                                                                                     teacher_uid_lists,
                                                                                                                                                                     workers_by_age_to_assign_count,
                                                                                                                                                                     potential_worker_uids,
                                                                                                                                                                     potential_worker_uids_by_age,
        ...
        workplace_rd_by_frontline = spdata.get_workplace_race_distr_by_frontline_status(**loc_pars)
        # Assign all workers who are not staff at schools to workplaces
        workplace_age_lists, workplace_uid_lists, potential_worker_uids, potential_worker_uids_by_age,potential_worker_uids_by_age_and_race, workers_by_age_to_assign_count = spw.assign_rest_of_workers(workplace_sizes,
                                                                                                                                                                   potential_worker_uids,
                                                                                                                                                                   potential_worker_uids_by_age,
                                                                                                                                                                   potential_worker_uids_by_age_and_race,
                                                                                                                                                                   workers_by_age_to_assign_count,
                                                                                                                                                                   ...
                                                                                                                                                                   cm_age_brackets,
                                                                                                                                                                   cm_age_by_brackets,
                                                                                                                                                                   contact_matrices,workplace_rd_by_frontline)
    ...
    def enrollment_rates_by_age(self):
        ...
        """
        return {k: self.information.enrollment_by_age[k]/self.information.age_count[k] if self.information.age_count[k] > 0 else 0 for k in range(
            defaults.settings.max_age)}
    ...
    def employment_rates_by_age(self):
        ...
        """
        return {k: self.information.employment_by_age[k]/self.information.age_count[k] if self.information.age_count[k] > 0 else 0 for k in range(
            defaults.settings.max_age)}
