...
def remove_ltcf_residents_from_potential_workers(facilities_by_uids, potential_worker_uids, potential_worker_uids_by_age,potential_worker_uids_by_age_and_race, workers_by_age_to_assign_count, age_by_uid,race_by_uid):
    """
    ...
    for nf, fc in enumerate(facilities_by_uids):
        for uid in fc:
            ...
            rindex = race_by_uid[uid]
            if uid in potential_worker_uids: # pragma: no cover
                potential_worker_uids_by_age[aindex].remove(uid)
                potential_worker_uids_by_age_and_race[rindex][aindex].remove(uid)
                potential_worker_uids.pop(uid, None)
                if workers_by_age_to_assign_count[aindex] > 0:
                    workers_by_age_to_assign_count[aindex] -= 1


    return potential_worker_uids, potential_worker_uids_by_age,potential_worker_uids_by_age_and_race, workers_by_age_to_assign_count
