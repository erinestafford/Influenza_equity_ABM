...
def assign_uids_by_homes(homes, race_probs_by_hh, id_len=16, use_int=True):
    """
    ...
    for h, home in enumerate(homes):
        race = np.random.choice(np.arange(5), p=race_probs_by_hh[len(home)])
        home_ids = []
        for a in home:
            if use_int:
                uid = len(age_by_uid)
            ...
            # TODO: interracial Households?
            race_by_uid[uid]=race
            home_ids.append(uid)

        homes_by_uids.append(home_ids)

    return homes_by_uids, age_by_uid, race_by_uid
...
def get_household_heads(popdict):
    ...
    for i, person in popdict.items():
        if popdict[i]['age']>= 18 and person['hhid'] is not None:
    ...
    for i, id in household_heads.items():
        if id==np.inf:
            print(id)
