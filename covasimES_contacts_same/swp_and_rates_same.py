...
if __name__ == '__main__':
	...
	s1.pars['prognoses']['symp_probs'] = np.array([[0.4016,0.50573333,0.52426, 0.51496667, 0.44964, 0.44964],
	...
	np.savetxt("results/swp_rs_5mil/infections/i_num"+str(int(ind))+".txt",infs)
	np.savetxt("results/swp_rs_5mil/symptomatic_infections/si_num" + str(int(ind)) + ".txt", symp_infs )
	np.savetxt("results/swp_rs_5mil/hrr/hrr"+str(int(ind))+".txt",hrr)
	np.savetxt("results/swp_rs_5mil/hospitalizations/h_num"+str(int(ind))+".txt",hosps)
	np.savetxt("results/swp_rs_5mil/icu/icu_num" + str(int(ind)) + ".txt", icus)
	np.savetxt("results/swp_rs_5mil/deaths/d_num"+str(int(ind))+".txt",deaths)
	np.savetxt("results/swp_rs_5mil/r_eff/reff"+str(int(ind))+".txt",s1.results['r_eff'])
	np.savetxt("results/swp_rs_5mil/pop/pop"+str(int(ind))+".txt",pops)
