...
class People(cvb.BasePeople):
    ...
    **Examples**::
    ...
    def set_prognoses(self):
        ...
        rinds=np.digitize(self.race, [0,1,2,3,4])-1
        #inds is ages
        self.symp_prob[:]   = progs['symp_probs'][rinds,inds]
        self.severe_prob[:] = progs['severe_probs'][rinds, inds]
        self.crit_prob[:] = progs['crit_probs'][rinds, inds]
        self.death_prob[:] = progs['death_probs'][rinds, inds]
        self.symp_prob[:] = progs['symp_probs'][rinds, inds]
        self.rel_sus[:]     = progs['sus_ORs'][rinds, inds]
    ...
    def story(self, uid, *args):
        ...
        **Example**::
        ...
        for uid in uids:
            ...
            intro = f'\nPerson {uid}, a {p.age:.0f} year old {sex} is {p.race}'

            if not p.susceptible:
                if np.isnan(p.date_symptomatic):
                    print(f'{intro} and had asymptomatic COVID.')
                else:
                    print(f'{intro} and had symptomatic COVID.')
            else:
                print(f'{intro} and did not contract COVID.')
            total_contacts = 0
            no_contacts = []
